...
//        }
    ...
    /**
     * 로그인 처리 및 결과 알려주는 화면으로 이동 ( 2023.09.21 )
     */
    @ResponseBody
    ...
    public MsgDTO loginProc(HttpServletRequest request, HttpSession session) {
        ...
        try {
            ...
            // 로그인을 위해 아이디와 비밀번호가 일히하는지 확인하기 위한 userInfoService 호출하기
    ...
    @GetMapping(value = "userInfoSession")
    public String userInfoSession(HttpSession session, ModelMap model) throws Exception {

        // 로그 찍기
        log.info(this.getClass().getName() + ".user/userInfoSession Start !");
        ...
        log.info(this.getClass().getName() + ".user/userInfoSession End !");

        return "user/userInfoSession";
    }
    ...
    public String searchUserIdProc (HttpServletRequest request, ModelMap model ) throws Exception {
        ...
        */
        UserInfoDTO pDTO = new UserInfoDTO();
        pDTO.setUserName(userName);
        pDTO.setEmail(EncryptUtil.encAES128CBC(email)); // DB에 이메일은 암호화되어 저장되기 때문에 찾기 위해 암호화
    ...
    */
     ...
     */
    @ResponseBody
    @PostMapping("getEmailExistsPs")
    public UserInfoDTO getEmailExistsPs(HttpServletRequest request) throws Exception {

        log.info(this.getClass().getName() + ".getEmailExistsPs Start !");
        ...
        // 입력된 이메일이 중복된 이메일인지 조회
        UserInfoDTO rDTO = Optional.ofNullable(userInfoService.getEmailExistsPs(pDTO)).orElseGet(UserInfoDTO::new);

        log.info(this.getClass().getName() + ".getEmailExistsPs End !");
